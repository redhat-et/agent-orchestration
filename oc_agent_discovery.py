...
def discover_routes(namespace_flag: str) -> List[Dict[str, Any]]:
    """Discover A2A routes using OpenShift CLI."""
    try:
        # Run oc command to get routes with A2A label
        cmd = f"oc get routes {namespace_flag} -l a2a.agent=true -o json"
        result = subprocess.run(
            cmd.split(),
            capture_output=True,
            text=True,
...
def discover_agents(namespace: Optional[str] = None, all_namespaces: bool = False, verify_endpoints: bool = False) -> str:
    """Discover A2A agents in OpenShift cluster or namespace."""
    ...
    # Discover routes
    routes = discover_routes(namespace_flag)

    if not routes:
        return f"No A2A agents found in {scope_msg}.\n\nTo make agents discoverable, ensure they have these labels:\n  a2a.agent: \"true\"\n  a2a.agent.name: \"your-agent-name\""
    ...
    for route in routes:
        ...
        agent_name = metadata.get("labels", {}).get("a2a.agent.name", "unknown")
        namespace = metadata.get("namespace", "")
        route_name = metadata.get("name", "")
        host = spec.get("host", "")
        endpoint = metadata.get("annotations", {}).get("a2a.agent.endpoint", "/.well-known/agent.json")
        description = metadata.get("annotations", {}).get("a2a.agent.description", "")

        url = f"https://{host}"
        agent_card_url = f"{url}{endpoint}"
    ...
    result_text = f"Found {len(agents)} A2A agent(s) in {scope_msg}:\n\n"
...
def list_agents(namespace: Optional[str] = None, all_namespaces: bool = False) -> str:
    """Get a summary list of all discovered A2A agents."""
    ...
    try:
        ...
        if "No A2A agents found" in agents_json or "Error:" in agents_json:
            return agents_json
        ...
        summary = "A2A Agent Summary:\n\n"
        summary += f"{'AGENT NAME':<20} {'NAMESPACE':<15} {'URL':<50} {'DESCRIPTION':<30}\n"
        summary += f"{'-'*20} {'-'*15} {'-'*50} {'-'*30}\n"

        for agent in agents:
            summary += f"{agent['agent_name']:<20} {agent['namespace']:<15} {agent['url']:<50} {agent['description']:<30}\n"
